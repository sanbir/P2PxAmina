# P2PxAmina — Canonical Smart Contract Architecture

**Version**: v0.3 (Claude, 2026-05-26)
**Status**: design reference for engineering, audit, AMINA integration, and counsel review
**Prior art**: consolidates `P2PxAmina-lending-protocol-for-banks.html` (v0.7 brief), `P2PxAmina-lending-protocol-for-banks-Contracts.html` (v0.1 sketch), `P2PxAmina-lending-protocol-for-banks-Implementation-Plan.md` (v0.2 plan), `GPT-thoughts.md`, and `Claude-thoughts-1.md`.

This document is intended to be the **single canonical architecture reference** for the project. If a downstream document (PR, audit brief, integration spec) disagrees with this one, this one wins or this one needs to be updated first.
...
| Roles | 8 |
| Invariants | 19 |
| Upgrade policy | `DealRegistry`, `EscrowVault`, `ParameterArchive` immutable; rest UUPS + timelock |
...
7. **Immutability where it matters.** `DealTerms` are write-once. `DealRegistry`, `EscrowVault`, and `ParameterArchive` are non-upgradeable contracts. The state-machine engine and policy registries are upgradeable behind a timelocked multisig.
...
flowchart TB
    ...
    subgraph L1["L1 — Identity &amp; Registry"]
        RM["RoleManager<br/>~40 LOC · UUPS<br/><i>OZ AccessManager wrapper</i>"]
...
| **Upgrade** | UUPS via OZ proxy. Itself controlled by the `GOVERNOR` role. |
...
| **Key invariant** | Tokens are typed as either `Supply` or `Collateral`; a given token cannot be used for both purposes in the same protocol deployment. |
...
| **Storage** | `mapping(bytes32 => mapping(uint32 => Params)) archive;` |
| **Upgrade** | **Immutable.** Written once per (pair, version) tuple. |
| **Public surface** | `write(pair, version, params)` (only `CollateralRegistry`), `read(pair, version)` (public view). |
...
| **Key invariant** | `sum over deals of balanceOf[d][token] == IERC20(token).balanceOf(address(this))` at the end of every external call. |
...
| **Upgrade** | UUPS (so the event schema can evolve). |
...
sequenceDiagram
    ...
    LendingEngine->>LendingEngine: state.versionKey points to new override entry
    LendingEngine->>SettlementRouter: emit OracleOverridden(dealId, ...)
    Note over LendingEngine: This is the ONLY function that<br/>can modify a live deal's effective terms.<br/>Cannot change LTV, rate, parties, principal.
```
...
- **Emergency rebind (oracle source only)**: `EMERGENCY.forceOracleOverride` is the only way to alter a live deal's effective oracle, and it cannot alter any other parameter.
...
|---|---|---|---|---|
| Global notional | Single blast-radius cap on the protocol | `LendingEngine` | $5B | OPS / CURATOR |
| Per supply token | Concentration in one supply asset | `IssuerRegistry` | $3B | OPS / CURATOR |
| Per collateral token | Concentration in one collateral asset | `IssuerRegistry` | $2B | OPS / CURATOR |
...
| Per 30d / 60d / 90d / 180d / 365d maturity bucket | Tenor concentration | `LendingEngine` | $500M each | OPS / CURATOR |
...
| Bug in `EscrowVault` | Halt protocol; declare emergency; one-way evacuation script transfers each deal's balance to a freshly-deployed `EscrowVault`; redeploy engine pointed at new vault. **Hard but possible.** |
...
14. **Vault reconciliation**: `sum over deals of EscrowVault.balanceOf[d][token] == IERC20(token).balanceOf(EscrowVault)` at the end of every external call.
...
End of canonical architecture v0.3.
